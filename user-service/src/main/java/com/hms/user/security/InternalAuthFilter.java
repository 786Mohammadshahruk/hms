...
/**
 * Replaces JwtAuthenticationFilter once gateway-enforced security is active.
 *
 * The API Gateway:
 *   1. Validates the JWT
 *   2. Injects X-User-Id, X-User-Email, X-User-Role headers
 *   3. Stamps every forwarded request with X-Internal-Secret
 *
 * This filter trusts those headers only when the secret matches.
 * Direct calls to the service port (without the gateway) are rejected with 403.
 */
@Slf4j
@Component
